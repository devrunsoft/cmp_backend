...
{
    public class AdminUpdateInvoiceHandler : IRequestHandler<AdminUpdateRequestCommand, CommandResponse<RequestResponse>>
    {
        ...
        public async Task<CommandResponse<RequestResponse>> Handle(AdminUpdateRequestCommand requests, CancellationToken cancellationToken)
        {
            ...
            var invoice = (await _invoiceRepository.GetAsync(p => p.Id == requests.RequestId, query => query.Include(x => x.Company))).FirstOrDefault();
            bool canEdit = false;
            ...
            {
                return new NoAcess<RequestResponse>() { Message = "You cannot edit an invoice that is linked to a contract." };
            }
            var CompanyId = invoice.CompanyId;
            var services = (await _baseServiceAppointmentRepository.GetAsync(p => p.RequestId == invoice.Id, query => query.Include(x=>x.ServiceAppointmentLocations))).ToList();
            ...
            foreach (var request in requests.ServiceAppointment)
            {
                ...
                if (request.Id != null && request.Id != 0 && (request.tempId == null || request.tempId == 0))
                {
                   ...
                   .Select(id =>
                     {
                         if (serviceEntity.ServiceAppointmentLocations.Any(x => x.Id == id))
                         {
                             return serviceEntity.ServiceAppointmentLocations.FirstOrDefault(x => x.Id == id);
                         }
                         else
                         {
