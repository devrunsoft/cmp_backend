...
{
    public class AdminProviderUpdateRequestHandler : IRequestHandler<AdminProviderUpdateRequestCommand, CommandResponse<RequestResponse>>
    {
        ...
        public AdminProviderUpdateRequestHandler(IRequestRepository invoiceRepository, IProductPriceRepository productPriceRepository,
        ...
        public async Task<CommandResponse<RequestResponse>> Handle(AdminProviderUpdateRequestCommand requests, CancellationToken cancellationToken)
        {

            var invoice = (await _invoiceRepository.GetAsync(p => p.Id == requests.RequestId, query => query.Include(x => x.Company)
            )).FirstOrDefault();

            if (invoice.Status == InvoiceStatus.Send_Payment)
            {
                return new NoAcess<RequestResponse>() { Message = "No Access To Edit This Invoice" };
            }
            ...
            var services = (await _baseServiceAppointmentRepository.GetAsync(p => true , query => query.Include(x=>x.ServiceAppointmentLocations))).ToList();
